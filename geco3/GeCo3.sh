...
SHOW_MENU () {
  echo " -------------------------------------------------------";
  echo "                                                        ";
  echo " GeCo v3, v1.0. High reference-free compression of DNA  ";
  ...
  echo "                                                        ";
  echo " -l <INT>, --level <INT>      GeCo3 compression level,  ";
  ...
  echo " Example -----------------------------------------------";
  echo "                                                        ";
  echo " ./GeCo3.sh --block 16MB -t 8 -i sample.seq             ";
  echo " ./GeCo3.sh --decompress -t 4 -i sample.seq.tar         ";
  echo "                                                        ";
  echo " -------------------------------------------------------";
  }
#
...
Program_installed () {
  if ! [ -x "$(command -v $1)" ];
    ...
    echo -e "\e[42mTIP\e[49m: Try: ./GeCo3.sh --install" >&2;
    exit 1;
    fi
  }
#
...
SHOW_HEADER () {
  echo "                                                        ";
  echo " [GeCo3 :: DNA, FASTA, and FASTQ Extensions]           ";
  echo "                                                        ";
...
MERGE_DNA () {
  ...
  for file in "${FILES[@]}" #
    do
    GeCo3 -d $file 2> .tmp_report_$file &
...
SPLIT_DNA () {
  ...
  for file in "${FILES[@]}" #
    do
    GeCo3 -tm 4:1:0:0:0.82/0:0:0 -tm 7:10:1:0:0.85/0:0:0 -tm 13:100:1:1:0.95/0:0:0 -tm 18:1000:1:15:0.95/5:10:0.95 $file 2> .tmp_report_$file &
    if [[ "$IDX_T" -eq "$THREADS" ]] || [[ "${#FILES[@]}" -eq "$IDX_POS" ]]
      then
      wait;
      IDX_T=0;
...
while [[ $# -gt 0 ]]
  ...
  case $i in
    ...
    -d|--decompress|--uncompress|--decompression)
      ...
      shift
    ;;
    -l|--level|--compression-level|--geco-level|--mode)
      LEVEL="$2";
      shift 2;
    ;;
    -b|--block)
